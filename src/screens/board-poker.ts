...
export function renderPokerBoard(state: PokerViewState): string {
  ...
  return `
    <div class="fn-board fn-board-poker">
      ${opponentLeft ? `<div class="fn-side left">${renderPlayerBadge(opponentLeft)}<div class="fn-poker-chips">${opponentLeft.chips}V</div></div>` : ''}
      ${opponentRight ? `<div class="fn-side right">${renderPlayerBadge(opponentRight)}<div class="fn-poker-chips">${opponentRight.chips}V</div></div>` : ''}
      ...
      <div class="fn-board-center">
          ...
          <div class="fn-poker-community">
            ${Array.from({ length: 5 })
              .map((_, i) => {
                ...
                return card
                  ? `<div class="fn-bj-card community">${card.value}${card.suit}</div>`
                  : '<div class="fn-bj-card placeholder"></div>';
              })
              .join('')}
          </div>
        ...
        <div class="fn-player-area bj-player">
          <div class="fn-poker-player-hand">
            ${state.playerHand.map((c) => `<div class="fn-bj-card player-card">${c.value}${c.suit}</div>`).join('')}
