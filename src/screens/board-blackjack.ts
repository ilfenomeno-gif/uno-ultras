...
export function renderBlackjackBoard(state: BlackjackViewState): string {
  if (state.phase === 'bet') {
    return `
      <div class="fn-board fn-board-bj">
  ...
  return `
    <div class="fn-board fn-board-bj">
      ...
      ${opponentLeft ? `<div class="fn-side left">${renderPlayerBadge(opponentLeft)}<div class="fn-bj-side-score">${opponentLeft.score}</div></div>` : ''}
      ${opponentRight ? `<div class="fn-side right">${renderPlayerBadge(opponentRight)}<div class="fn-bj-side-score">${opponentRight.score}</div></div>` : ''}
      ...
      <div class="fn-board-center">
        ...
        <div class="fn-bj-dealer-area">
          <div class="fn-bj-hand dealer">
            ${state.dealerHand
              .map((c) => `<div class="fn-bj-card ${c.hidden ? 'hidden' : ''}">${c.hidden ? '?' : `${c.value}${c.suit}`}</div>`)
              .join('')}
          </div>
